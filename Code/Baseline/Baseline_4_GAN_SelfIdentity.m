...
test=Table[<|"P_GCM"->nP4GCM[[i]],
             "P_Obser"->nP4Obser[[i]]|>,{i,length+vlength+1,Length[nP4GCM]}];
...
validation=Table[<|"P_GCM"->nP4GCM[[i]],"P_Obser"->nP4Obser[[i]]|>,{i,length,length+vlength}];
...
discriminatorGCM2Obser = NetChain[{
...
ReportCycleGan[net_] :=
  Block[{gen,dGCM,dObser,obserG,obserD,gcmD,meanDiff,varDiff,dlossGCM,dlossObser},
        ...
        obserG=Map[gen[<|"P"->#[["P_GCM"]]|>,TargetDevice->"GPU"]&,validation];
        ...
        If[meanDiff+varDiff<=DiffMean+DiffVar,
          Block[{},
		Print[index];
                Export["/g/g92/pan11/Baseline_4_GAN_SelfIdentity_"<>index<>".mx",net];
...
NetTrain[gan,
    ...
    LossFunction ->{"FakeLoss_GCM->Obser"->Scaled[1],"RealLoss_GCM->Obser"->Scaled[1],"Loss_GCM2Obser_SelfRegression"->Scaled[-hype[[3]]]},
    TrainingUpdateSchedule -> {"Discriminator_GCM->Obser","Generator_GCM->Obser","Generator_GCM->Obser_SelfRegression"},
    LearningRateMultipliers -> {"Scale_GCM->Obser" -> 0, 
                             "Generator_GCM->Obser" -> -1,"Discriminator_GCM->Obser"->1,"Generator_GCM->Obser_SelfRegression"->-1},
    BatchSize -> 32,
    TargetDevice->"GPU",
    MaxTrainingRounds->100,
    Method -> {"ADAM", "Beta1" -> 0.5, "LearningRate" -> 10^-4,
                           "WeightClipping" -> {"Discriminator_GCM->Obser"->hype[[2]]/100.}},
    TrainingProgressReporting -> {{Function@ReportCycleGan[#Net], "Interval" -> Quantity[1, "Rounds"]},"Print"}]

gan=Import["/g/g92/pan11/Baseline_4_GAN_SelfIdentity_"<>index<>".mx"];
NetTrain[gan,
    {Function[Block[{base,choice,choice2},
        base=RandomSample[Range[2,length],#BatchSize];
        choice=Map[Block[{daylag=RandomSample[Range[-15,15]][[1]],yearlag=RandomSample[Range[-5,5]][[1]],tempt},
