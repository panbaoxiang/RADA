...
(*
energy0=Block[{correction=gcm,rcorrection=RandomSample[correction]},
energy=2*Total[Flatten[Table[EuclideanDistance[correction[[i]],obser[[j]]],{i,Length[correction]},{j,Length[obser]}]]]/Length[correction]/Length[obser]-Total[Flatten[Table[EuclideanDistance[robser[[i]],obser[[j]]],{i,Length[correction]},{j,Length[obser]}]]]/Length[correction]/Length[obser]-Total[Flatten[Table[EuclideanDistance[rcorrection[[i]],correction[[j]]],{i,Length[correction]},{j,Length[obser]}]]]/Length[correction]/Length[obser]];
Export["/usr/workspace/pan11/CycleGAN/MBCn/Step_0.mx",<|"energy"->energy0|>];
*)
...
Print[energyTest];*)
energyTest=2*Total[Flatten[Table[EuclideanDistance[correctionTest[[i]],tobser[[j]]],{i,Length[tobser]},{j,Length[tobser]}]]]/Length[tobser]/Length[tobser]-Total[Flatten[Table[EuclideanDistance[tobser[[i]],tobser[[j]]],{i,Length[tobser]},{j,Length[tobser]}]]]/Length[tobser]/Length[tobser]-Total[Flatten[Table[EuclideanDistance[correctionTest[[i]],correctionTest[[j]]],{i,Length[tobser]},{j,Length[tobser]}]]]/Length[tobser]/Length[tobser];
Print[energyTest];
...
*)
energyValidation=2*Total[Flatten[Table[EuclideanDistance[correctionValidation[[i]],vobser[[j]]],{i,Length[vobser]},{j,Length[vobser]}]]]/Length[vobser]/Length[vobser]-Total[Flatten[Table[EuclideanDistance[vobser[[i]],vobser[[j]]],{i,Length[vobser]},{j,Length[vobser]}]]]/Length[vobser]/Length[vobser]-Total[Flatten[Table[EuclideanDistance[correctionValidation[[i]],correctionValidation[[j]]],{i,Length[vobser]},{j,Length[vobser]}]]]/Length[vobser]/Length[vobser];
Print[energyValidation];

Print[{"Step ",kk,energyTraining,energyValidation,energyTest}];
Export["/usr/workspace/pan11/CycleGAN/MBCn/Step_"<>ToString[kk]<>".mx",
<|"transform"->transform,
  "matrix"->matrix,
